const Classes = require('../../classes')

const oneAction = [
	{
		icon:
			'<path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M11.8709 12.7801L8.77759 13.2226L9.21925 10.1284L17.1743 2.17345C17.9065 1.44121 19.0937 1.44121 19.8259 2.17345C20.5582 2.90568 20.5582 4.09288 19.8259 4.82511L11.8709 12.7801Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M16.29 3.05762L18.9417 5.70928" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M16.625 12.875V19.125C16.625 19.8154 16.0654 20.375 15.375 20.375H2.875C2.18464 20.375 1.625 19.8154 1.625 19.125V6.625C1.625 5.93464 2.18464 5.375 2.875 5.375H9.125" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'
	}
]

const twoActions = [
	{
		icon:
			'<path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M11.8709 12.7801L8.77759 13.2226L9.21925 10.1284L17.1743 2.17345C17.9065 1.44121 19.0937 1.44121 19.8259 2.17345C20.5582 2.90568 20.5582 4.09288 19.8259 4.82511L11.8709 12.7801Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M16.29 3.05762L18.9417 5.70928" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M16.625 12.875V19.125C16.625 19.8154 16.0654 20.375 15.375 20.375H2.875C2.18464 20.375 1.625 19.8154 1.625 19.125V6.625C1.625 5.93464 2.18464 5.375 2.875 5.375H9.125" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'
	},
	{
		icon:
			'<path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M14.375 17.5H5.625C4.93464 17.5 4.375 16.9404 4.375 16.25V5H15.625V16.25C15.625 16.9404 15.0654 17.5 14.375 17.5Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M8.125 13.75V8.75" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M11.875 13.75V8.75" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.875 5H18.125" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M11.875 2.5H8.125C7.43464 2.5 6.875 3.05964 6.875 3.75V5H13.125V3.75C13.125 3.05964 12.5654 2.5 11.875 2.5Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'
	}
]

const threeActions = [
	{
		icon:
			'<path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M11.8709 12.7801L8.77759 13.2226L9.21925 10.1284L17.1743 2.17345C17.9065 1.44121 19.0937 1.44121 19.8259 2.17345C20.5582 2.90568 20.5582 4.09288 19.8259 4.82511L11.8709 12.7801Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M16.29 3.05762L18.9417 5.70928" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M16.625 12.875V19.125C16.625 19.8154 16.0654 20.375 15.375 20.375H2.875C2.18464 20.375 1.625 19.8154 1.625 19.125V6.625C1.625 5.93464 2.18464 5.375 2.875 5.375H9.125" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'
	},
	{
		icon:
			'<path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M14.375 17.5H5.625C4.93464 17.5 4.375 16.9404 4.375 16.25V5H15.625V16.25C15.625 16.9404 15.0654 17.5 14.375 17.5Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M8.125 13.75V8.75" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M11.875 13.75V8.75" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.875 5H18.125" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M11.875 2.5H8.125C7.43464 2.5 6.875 3.05964 6.875 3.75V5H13.125V3.75C13.125 3.05964 12.5654 2.5 11.875 2.5Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'
	},
	{
		icon:
			'<path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M3.8125 12.2065C5.02062 12.2065 6 11.2272 6 10.019C6 8.81092 5.02062 7.83154 3.8125 7.83154C2.60438 7.83154 1.625 8.81092 1.625 10.019C1.625 11.2272 2.60438 12.2065 3.8125 12.2065Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M18.1875 12.2065C19.3956 12.2065 20.375 11.2272 20.375 10.019C20.375 8.81092 19.3956 7.83154 18.1875 7.83154C16.9794 7.83154 16 8.81092 16 10.019C16 11.2272 16.9794 12.2065 18.1875 12.2065Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M11 12.2065C12.2081 12.2065 13.1875 11.2272 13.1875 10.019C13.1875 8.81092 12.2081 7.83154 11 7.83154C9.79188 7.83154 8.8125 8.81092 8.8125 10.019C8.8125 11.2272 9.79188 12.2065 11 12.2065Z" stroke="#0076D6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'
	}
]

module.exports = {
	title: 'Text List',
	collated: true,
	collator: function(markup, item) {
		return `<!-- Start: @${
			item.handle
		} -->\n<div style="margin-bottom: 2rem;">${markup}</div>\n<!-- End: @${
			item.handle
		} -->\n`
	},
	context: {
		parentClass: 'text-list',
		listHeaderTitle: 'Text List',
		items: [
			{
				title: 'Send me a reminder before each shift'
			}
		]
	},
	variants: [
		{
			name: 'Subtitle',
			context: {
				listHeaderTitle: 'Holidays',
				items: [
					{
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed'
					}
				]
			}
		},
		{
			name: 'Actions',
			context: {
				listHeaderTitle: 'Actions',
				items: [
					{
						title: 'Send me a reminder before each shift',
						actions: oneAction
					},
					{
						title: 'Send me a reminder before each shift',
						actions: twoActions
					},
					{
						title: 'Send me a reminder before each shift',
						actions: threeActions
					}
				]
			}
		},
		{
			name: 'Actions',
			context: {
				listHeaderTitle: 'Actions',
				items: [
					{
						title: 'Send me a reminder before each shift',
						isDraggable: true,
						actions: oneAction
					},
					{
						title: 'Send me a reminder before each shift',
						isDraggable: true,
						actions: twoActions
					},
					{
						title: 'Send me a reminder before each shift',
						isDraggable: true,
						actions: threeActions
					}
				]
			}
		},
		{
			name: 'Actions + Subtitle',
			context: {
				listHeaderTitle: 'Actions',
				items: [
					{
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed',
						actions: oneAction
					},
					{
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed',
						actions: twoActions
					},
					{
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed',
						actions: threeActions
					}
				]
			}
		},
		{
			name: 'Icon',
			context: {
				listHeaderTitle: 'Important Dates',
				items: [
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed'
					},
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						title: 'Thu, Nov 29, 2018',
						subtitle: 'Closed'
					},
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						title: 'Fri, Nov 30, 2018',
						subtitle: '10am–10pm'
					}
				]
			}
		},
		{
			name: 'Icon + Draggable',
			context: {
				listHeaderTitle: 'Important Dates',
				items: [
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						isDraggable: true,
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed'
					},
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						isDraggable: true,
						title: 'Thu, Nov 29, 2018',
						subtitle: 'Closed'
					},
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						isDraggable: true,
						title: 'Fri, Nov 30, 2018',
						subtitle: '10am–10pm'
					}
				]
			}
		},
		{
			name: 'Small',
			context: {
				listHeaderTitle: 'Important Dates',
				isSmall: true,
				items: [
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						title: 'Wed, Nov 28, 2018',
						subtitle: 'Closed',
						actions: threeActions
					},
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						title: 'Thu, Nov 29, 2018',
						subtitle: 'Closed',
						actions: threeActions
					},
					{
						image:
							'<path fill="none" d="M6.625 11.625H7.25C7.59518 11.625 7.875 11.9048 7.875 12.25V16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 16.625H9.125" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M12.5266 11.625H14.1249C14.3296 11.6259 14.5208 11.7269 14.6371 11.8953C14.7534 12.0637 14.7799 12.2783 14.7083 12.47L13.1499 16.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" fill-rule="evenodd" clip-rule="evenodd" d="M1.625 5.625C1.625 4.79657 2.29657 4.125 3.125 4.125H18.875C19.7034 4.125 20.375 4.79657 20.375 5.625V18.875C20.375 19.7034 19.7034 20.375 18.875 20.375H3.125C2.29657 20.375 1.625 19.7034 1.625 18.875V5.625Z" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M1.625 9.125H20.375" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M6.625 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path fill="none" d="M15.375 6V1.625" stroke="#737780" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
						iconClass: 'list-item__icon',
						imageIsIcon: true,
						title: 'Fri, Nov 30, 2018',
						subtitle: '10am–10pm',
						actions: threeActions
					}
				]
			}
		}
	]
}
